r"""Tiny JSON-backed config for the CryptBox GUI.

Stores just two things and never raises: the chosen theme ("light"/"dark") and a
short list of recent input/output paths.  On Windows the file lives at
``%LOCALAPPDATA%\CryptBox\config.json``; elsewhere it falls back to
``~/.cryptbox/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\CryptBox`` on Windows, ``~/.cryptbox`` otherwise.
    """
